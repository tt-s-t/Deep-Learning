...
class Model(torch.nn.Module):
    def __init__(self,num_inputs,hidden_size1,hidden_size2,num_outputs):
        ...
        self.layer = torch.nn.Sequential(
            torch.nn.Linear(num_inputs,hidden_size1),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_size1,hidden_size2),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_size2,num_outputs),
            torch.nn.Softmax()
        )
...
print(params)#输出网络参数
torch.save(model,'model.pth')#保存网络(网络结构+参数)
#torch.save(model.state_dict(), 'net_params.pth')#保存网络(参数)
model.eval()
test_correct = 0
...
########################加载保存的模型进行测试##################
model=torch.load('model.pth')#保存网络(网络结构+参数)
#model=Model(num_inputs,hidden_size1,hidden_size2,num_outputs)
