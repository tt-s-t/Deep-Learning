...
def train(dataset, model, args):
    ...
    for epoch in range(args.max_epochs):
        h_state = torch.zeros(model.num_layers, args.sequence_length, model.hidden_size)
        c_state = torch.zeros(model.num_layers, args.sequence_length, model.hidden_size)
        epoch_loss = 0
        for _, (x, y) in enumerate(train_loader):
            y_pred, (h_state, c_state) = model(x.to(device), (h_state.to(device), c_state.to(device)))
            
            loss = criterion(y_pred.transpose(1, 2), y.to(device)) #y_pred.transpose(1, 2)第一二维倒置
            h_state = h_state.detach() #返回一个新的tensor，从当前计算图中分离下来。但是仍指向原变量的存放位置，不同之处只是requirse_grad为false.得到的这个tensor永远不需要计算梯度，不具有grad.这样我们的h_state就不会在更新参数的时候被改变了。
...
def predict(dataset, model, text, next_words_len, args):
    for j in range(1,5):#写四章小说，每章next_words_len字
        ...
        device = 'cpu'
        model.to(device)
        h_state = torch.zeros(model.num_layers, len(words), model.hidden_size)
        c_state = torch.zeros(model.num_layers, len(words), model.hidden_size)
